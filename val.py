...
def val(model, dataloader, epoch=9999):
    ...
    with torch.no_grad():
        for iter_idx, (images, labels) in enumerate(dataloader):
            ...
            print(f"[Epoch {epoch}] iter {iter_idx} / {n_iters}: Validation accuracy {acc_meter.val:.2f}({acc_meter.avg:.2f})", end='\r')
    print("")
    print(f"Epoch {epoch} validation: Accuracy {acc_meter.avg}")
